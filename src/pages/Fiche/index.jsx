...
const ContainerFiche = ({ children }) => (
  <div className="container-fiche">{children}</div>
);
const HeaderFiche = ({ children }) => (
  <div className="header-fiche">{children}</div>
);
const Column = ({ children, position }) => (
  <div className={"col-" + position}>{children}</div>
);
...
const Details = ({ description, equipements }) => {
  return (
    <div className="description-fiche">
      ...
      <TextCollapse title="Equipements">
        {equipements.map((e, index) => (
          <span key={"fiche_" + index}>
            {e}
            <br />
          </span>
        ))}
...
const Fiche = () => {
  ...
  if (!fiche) return <Navigate to="/404" />;
