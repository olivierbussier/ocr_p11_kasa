...
describe('Nav tests with url', () => {
  test('landing on a propos', () => {
    ...
    expect(screen.getByText(/La sécurité est la priorité de Kasa/i)).toBeInTheDocument()
  })
  ...
  test('landing on a file page', () => {
    ...
    // verify navigation to "no match" route
    expect(screen.getByText(/Profitez du charme de la vie/i)).toBeInTheDocument()
  })
...
describe("Nav tests with buttons", () => {
  ...
  test('Navigate to a card from home', async () => {
    ...
    expect(screen.getByText(/venez découvrir Paris dans ce studio tout équipé/i)).toBeInTheDocument()
    // check that the content changed to the new page
  });
})